...
@app.route("/api/v1/filaments/<int:id>/", methods=["GET"])
def filament(id: int):
    conn = get_conn()
    curs = conn.cursor()
...
@app.route("/api/v1/filaments/random/", methods=["GET"])
def filament_random():
    # get all available filaments
    response = requests.get(f"http://{IP}:5000/api/v1/info/")
    ...
    return requests.get(f"http://{IP}:5000/api/v1/filaments/{id}/").json()
...
def filament_post():
    ...
    if image and image.filename:
        ...
        curs.execute(
            ...
            (f"http://{IP}:5000/api/v1/images/filaments/{id}.{extension}", id),
        )
...
@app.route("/api/v1/filaments/<int:id>/", methods=["PUT"])
...
@app.route("/api/v1/filaments/<int:id>/", methods=["DELETE"])
